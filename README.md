...
**真实密钥不进 git**，全部收在一个本地文件 `~/code/.env` 里，`shellrc` 自动 `source` 它，安装脚本从 `env.example` 拷占位符版过去，由你填真实值：

| 本地文件 | 模板 | 放什么 |
|---|---|---|
| `~/code/.env` | `env.example` | 20+ 个 API key（OpenAI/Claude/火山/微信/ASC/Cloudflare/Tailscale…）+ `VOLC_APPID`、`MODEL_SPEECH_API_KEY`、`X_ACCESS_TOKEN` |
...
1. 填一个本地文件：`~/code/.env`（模板 `env.example`）
...
反代实现在网站仓库 `~/code/jianshuo.dev/functions/setup/` 下的
